...
$update_account_info = "SELECT first_name, last_name, email, phone, address_1, address_2, city, game_states_id, zip, date(date_created) as date_created
	FROM game_customers WHERE game_customers_id = $game_customers_id";
$exec_update_account_info = @mysqli_query($link, $update_account_info);
if(!$exec_update_account_info){
	rollback('Customer account info not retreived due to: '.mysqli_error($link));
